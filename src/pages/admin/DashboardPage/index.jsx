...
const DashboardPage = () => {
  ...
  if (loading) {
    return (
      <div>
        ...
        <p className="mt-10 text-center text-lg text-black/60">
          Carregando dados...
        </p>
      </div>
    );
  ...
  if (!data) {
    return (
      <div>
        <PageMarker name="DashboardPage" />
        <AdminTitle title="Dashboard" />
        <p className="mt-10 text-center text-lg text-[#ff3333]">
  ...
  const { sales_summary, customers_summary, recent_orders, low_stock_alerts } =
    data;
    ...
    <div>
      ...
      {low_stock_alerts && low_stock_alerts.length > 0 && (
        <AdminPanel className="mx-auto mt-6 max-w-[920px] border border-[#ff3333]/20 p-5 md:mt-14 md:p-8">
          <h2 className="text-[16px] font-bold uppercase text-[#ff3333] md:text-[20px]">
          ...
          <div className="mt-4 space-y-3">
            {low_stock_alerts.map((alert) => (
              <div
                key={alert.id}
                className="flex justify-between border-b border-black/10 pb-2 text-[14px] last:border-0 last:pb-0 md:text-base"
              >
                <span className="text-black/80">
                  {alert.product_name} — {alert.variation_size}
                </span>
                <span className="font-bold text-[#ff3333]">
                  {alert.stock_quantity} un.
                </span>
              </div>
            ))}
          </div>
      ...
      <AdminPanel className="mx-auto mt-6 max-w-[920px] p-5 md:mt-20 md:p-8">
        ...
        {chartData.length === 0 || chartData.every((d) => d.total === 0) ? (
          <p className="mt-6 text-[14px] text-black/40">
          ...
          <>
              ...
              {(() => {
                ...
                return chartData.map((d) => (
                  ...
                  >
                    ...
                    <div
                      className="w-full rounded-t-[4px] bg-[#1f1f1f] transition-all"
                      style={{
            ...
            <div className="mt-2 flex gap-2 pl-8 md:gap-6 md:pl-[52px]">
              {chartData.map((d) => (
                <p
                  key={d.key}
                  className="w-full text-center text-[10px] capitalize text-black/40 md:text-[12px]"
                >
                  {d.label}
                </p>
              ))}
